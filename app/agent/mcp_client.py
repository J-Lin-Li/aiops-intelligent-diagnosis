...
from langchain_mcp_adapters.client import MultiServerMCPClient #能同时连多个 MCP server
...
async def load_mcp_tools_safe(
    ...
    try:
        ...
        #1. 分别连接 8003 和 8004
        #2. 向每个 server 请求"你有哪些工具"（MCP 协议规定的发现接口）
        #3. 拿回工具列表和每个工具的 schema、描述
        #4. 把这些 MCP 工具转换成 LangChain BaseTool 格式
...
async def load_mcp_tools_per_server(
    ...
    """逐 server 加载工具，单个 server 失败不影响其他 server。

    Returns:
        (all_tools, errors) — errors 是 {server_name: 错误描述} 字典，全成功时为空。
    """
    target = servers or DEFAULT_MCP_SERVERS
    all_tools: list = []
    errors: Dict[str, str] = {}
    for name, cfg in target.items():
        try:
            client = MultiServerMCPClient({name: cfg}, tool_interceptors=[retry_interceptor])
            tools = await client.get_tools()
            all_tools.extend(tools)
            logger.info(f"MCP server [{name}] 加载 {len(tools)} 个工具")
        except BaseException as e:
            errors[name] = format_exception_chain(e)
            logger.warning(f"MCP server [{name}] 加载失败: {errors[name]}")
