...
from app.agent.mcp_client import load_mcp_tools_per_server
        ...
        (
            ...
            dedent("""
                ...
                **3. 'replan' - 当前计划有严重问题** 【最低优先级，谨慎使用】
                ...
                ⚠️ 工具失败不等于信息充足：
                - 如果已执行步骤的结果包含 "success: false"、"error"、"连接失败"、"connection refused" 等错误信息
                - 且已执行步骤数 < 3，剩余计划中还有其他工具可以尝试
                - 则应选择 continue，而不是 respond
                - 工具失败只代表"该渠道获取不到信息"，不代表"诊断任务已完成"
...
async def replanner(state: PlanExecuteState) -> Dict[str, Any]:
    ...
    mcp_tools, mcp_errors = await load_mcp_tools_per_server()
    if mcp_errors:
        logger.warning(f"部分 MCP server 加载失败: {mcp_errors}")
    ...
    #     if mcp_err:
    #         logger.warning(f"MCP 工具加载失败，降级为仅本地工具: {mcp_err}")
    #
    #     # 合并所有工具
    #     all_tools = local_tools + mcp_tools
    #     logger.info(f"可用工具数量: 本地 {len(local_tools)} + MCP {len(mcp_tools)}")
