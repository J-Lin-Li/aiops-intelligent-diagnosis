...
class MilvusClientManager:
    ...
    VECTOR_DIM: int = config.dashscope_embedding_dim  # embedding模型维度
    ...
    def _create_collection(self) -> None:
        ...
        fields = [
            ...
            FieldSchema(
                name="vector",
                dtype=DataType.FLOAT_VECTOR,# 768维(config.dashscope_embedding_dim 决定)
                dim=self.VECTOR_DIM,
            ),
            FieldSchema(
                name="content",
                dtype=DataType.VARCHAR,#原始文本片段
                max_length=self.CONTENT_MAX_LENGTH,
            ),
            FieldSchema(
                name="metadata",#来源文件名等元数据
                dtype=DataType.JSON,
            ),
        ]
