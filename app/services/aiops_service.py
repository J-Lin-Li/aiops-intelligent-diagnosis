...
from app.services.diagnosis_history_service import save_diagnosis
    ...
    async def execute(
        ...
        try:
            ...
            async for event in self.graph.astream(
                input=initial_state,
                config=config_dict,
                stream_mode="updates"# ← 节点级流式
            ):
                # 解析事件
                for node_name, node_output in event.items(): # 每次事件是一个节点的完整输出
            ...
            # 存储诊断历史到 MySQL
            await save_diagnosis(session_id, past_steps, final_response)
