    ...
    async def query_stream(
        ...
        try:
            ...
            async for token, metadata in self.agent.astream(
                ...
                #- token：当前这一片消息对象（可能是 AIMessageChunk、HumanMessage、ToolMessage 等）
                #- metadata：这片消息是哪个节点产生的（langgraph_node）

                #isinstance：metadata若是dict的实例
                node_name = metadata.get('langgraph_node', 'unknown') if isinstance(metadata, dict) else 'unknown'
                message_type = type(token).__name__

                if message_type in ("AIMessage", "AIMessageChunk"):
                    #getattr，获取对象的content_blocks，没有则输出None，若是查询写死的内容：‘’和.没区别，若是想将某个变量的值作为查询对象（可能可变），则用getattr
                    content_blocks = getattr(token, 'content_blocks', None)

                    # ChatQwen特有的结构。普通OpenAI直接拿token.content就是纯文本字符串。
                    # 但千问模型的消息可以包含多种内容块，比如：
                    # content_blocks = [
                    #     {"type": "thinking", "thinking": "让我先分析一下..."},# 思考链，不展示给用户
                    #     {"type": "text", "text": "根据文档，答案是..."} # 正式回答
                    #]
