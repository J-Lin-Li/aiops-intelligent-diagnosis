...
│   │   ├── diagnosis_history_service.py    # 诊断历史持久化（MySQL）
...
│   ├── tools/                              # Agent 工具集
│   │   ├── __init__.py
│   │   ├── knowledge_tool.py               # 知识库查询工具（含 Redis 缓存）
│   │   ├── query_metrics_alerts.py         # Prometheus 告警查询工具
│   │   └── time_tool.py                    # 时间工具
│   ├── core/                               # 核心组件
│   │   ├── __init__.py
...
├── .env                                    # 环境变量配置（复制 .env.example 后填入真实值）
├── .env.example                            # 环境变量模板
├── Makefile                                # 项目管理命令（Linux/macOS）
├── start-windows.bat                       # Windows 启动脚本
├── stop-windows.bat                        # Windows 停止脚本
├── docker-compose.yml                     # Milvus Docker Compose 配置
...
DASHSCOPE_EMBEDDING_MODEL=tongyi-embedding-vision-flash
DASHSCOPE_EMBEDDING_DIM=768
...
MCP_CLS_URL=http://localhost:18003/mcp
MCP_MONITOR_TRANSPORT=streamable-http
MCP_MONITOR_URL=http://localhost:18004/mcp

REDIS_HOST=localhost
REDIS_PORT=6379
REDIS_DB=0
REDIS_CACHE_TTL=3600
REDIS_CACHE_EMPTY_TTL=300
...
# 或者重启单个服务
docker compose -f docker-compose.yml restart standalone
```
...
# 检查端口占用
lsof -i :9900   # FastAPI
lsof -i :18003  # CLS MCP
lsof -i :18004  # Monitor MCP
```
...
netstat -ano | findstr :18003  # CLS MCP
netstat -ano | findstr :18004  # Monitor MCP
```
